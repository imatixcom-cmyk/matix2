...
RELAY_BUF = 64 * 1024         # سایز بافر رله (بایت) — بزرگ‌تر = تعداد سیستم‌کال کمتر
CONNECT_TIMEOUT = 10          # ثانیه، برای باز کردن اتصال TCP به مقصد
SOCK_BUF = 256 * 1024         # سایز بافر سوکت (SO_RCVBUF/SO_SNDBUF)


def _tune_socket(writer: asyncio.StreamWriter):
    """Nagle رو خاموش می‌کنه و بافر سوکت رو بزرگ می‌کنه — تاخیر رو به‌شدت کم می‌کنه."""
    try:
        ...
        logger.debug(f"socket tuning skipped: {e}")
...
async def run_tunnel(ws: WebSocket, is_uuid_allowed, on_bytes=None):
    ...
    task_a = asyncio.ensure_future(relay_ws_to_tcp(ws, writer, on_bytes=_count))
    task_b = asyncio.ensure_future(relay_tcp_to_ws(ws, reader, on_bytes=_count))

    # به محض تموم شدن یکی از دو جهت، اون یکی رو هم فوراً می‌بندیم —
    # به‌جای اینکه منتظر EOF طبیعی بمونیم (این تاخیر رو موقع قطع/ری‌کانکت کم می‌کنه)
